...
**สถานะ:** ทำเสร็จแล้วเฉพาะแกนหลัก — **ลากรวมเป็น tab / ลากแยกออกที่ขอบ ทำงานจริงข้ามทั้ง 3 โซน (ซ้าย/ขวา/ล่าง)** พร้อม resize และ drop-zone highlight — แต่ **ยังไม่ persist ลง `editor_layout.ron`** และยังไม่มี floating window (ตามสโคป v1 เดิม) ไฟล์ใหม่ [`bv_editor_ui::dock`](../crates/bv_editor_ui/src/dock.rs) (tree data model + drag/drop logic ล้วนๆ ไม่มี `bevy_ui`) และการ render ทั้งหมดอยู่ใน [`shell.rs`](../crates/bv_editor_ui/src/shell.rs) ที่ rewrite ใหม่:

- **ต่างจากแผนเดิมที่สุด: ไม่ได้ทำ `PanelDescriptor`/extension-panel ทั่วไปตามที่ DESIGN.md section 5 พูดถึง** — `PanelId` เป็น enum ปิด 4 ตัว (`Scene`/`Inspector`/`Assets`/`Console`) ไม่ใช่ string/registry แบบเปิด เพราะ `bv_editor_extension` (Phase 7, ที่ `PanelDescriptor` ควรอยู่) ยังเป็น crate เปล่าอยู่ — ทำ docking ให้รองรับ panel ที่ยังไม่มีอยู่จริงตอนนี้เป็นการ design ล่วงหน้าเกินจำเป็น เมื่อ Phase 7 เริ่มค่อยขยาย `PanelId` เป็น dynamic id จริง
- **โครงสร้าง tree เป็น binary split ไม่ใช่ N-ary `Split(axis, [children], sizes)` ตามที่สเปกเดิมร่างไว้** — `DockNode::Split { axis, first: Box<DockNode>, second: Box<DockNode>, first_size_px }` ให้ `first` มีขนาด px ตายตัว (ลาก `Splitter` ปรับได้) และ `second` เป็น `flex_grow: 1.0` เติมที่ว่างที่เหลือ — โครงสร้างเดียวกับที่ Scene Tree/Viewport/Components แถวกลางใช้อยู่แล้วตั้งแต่ Phase 1 เป๊ะๆ ทำให้ **ใช้ `Splitter`/`resize_value` เดิมได้ตรงๆ ไม่ต้องเขียนกลไก resize ใหม่เลย**
- **3 โซนอิสระ ไม่ใช่ dock tree เดียวครอบทั้ง shell** — `DockLayout { left, right, bottom }` แต่ละอันเป็น `DockNode` แยกกัน คนละ resource field, ไม่มี tree เดียวที่ครอบ viewport (viewport ไม่ dockable ตามคำขอเดิมข้อ 5 ที่พูดถึงแค่ "side panel") — ลาก panel ข้ามโซนได้ (เช่น ลาก Console จาก bottom ไปรวมกับ Scene Tree ทางซ้าย) เพราะ `DockLayout::merge_onto`/`split_onto` หา region ปลายทาง/ต้นทางจาก `PanelId` เองอัตโนมัติ ไม่ผูกกับโซนตายตัว
- **invariant ที่ตั้งใจไว้: โซนห้ามว่างเปล่า** — ถ้าลาก panel ตัวสุดท้ายออกจากโซนหนึ่งไปโซนอื่น จะถูกปฏิเสธ (no-op) เพราะยังไม่ได้ทำกลไก "โซนว่าง = ยุบซ่อนตัวเอง" (ซึ่งจะทำให้โซนที่ว่างกลายเป็น drop target ไม่ได้อีกเลยเพราะมองไม่เห็น — ปัญหาวนลูปที่ตั้งใจเลี่ยงไม่แก้ตอนนี้) ผลคือ: การลาก panel ระหว่างโซนบางลำดับอาจถูกปฏิเสธถ้าโซนต้นทางเหลือ panel เดียว ต้องรวม panel อื่นเข้าโซนนั้นก่อนถึงจะย้ายตัวที่เหลือออกได้ (ดูเทส `every_panel_can_end_up_in_a_different_region_than_it_started_in` ใน `dock.rs` ที่ trace ลำดับที่ใช้ได้จริงไว้)
- **drop-zone indicator ทำแบบง่ายกว่าที่ร่างไว้: ไฮไลต์ตัว drop target เอง แทนกล่อง overlay แยก** — เพราะ tab strip button (สำหรับ merge, `DockZone::Center`) กับแถบขอบบางๆ 4 ด้านของ content mount (สำหรับ split, `DockZone::Edge`) ต่างก็มีรูปทรง/ตำแหน่งที่สื่อความหมายอยู่แล้วในตัว ("ทับกลาง = ทั้งแถบ tab เด่นขึ้น" vs "ชิดขอบ = แถบบางเด่นขึ้นเฉพาะขอบนั้น") ไม่ต้องคำนวณตำแหน่ง/ขนาด overlay แยกเพิ่ม — ระบบ `dock_drop_target_highlight_system` เดียวจัดการทั้ง tab-active/tab-inactive/drop-hover ให้ครบ
- **title bar จริงมาในรูปแบบ tab strip button** ต่อ panel (ก่อนหน้านี้มีแค่ label ลอยเป็น child ของกล่อง ไม่ใช่ drag handle) — แต่ละปุ่มเป็นทั้ง `DockTabButton`(คลิกสลับ tab), `DragSource` (`DragPayload::Panel`), และ `DockDropTarget` (merge zone) ในตัวเดียว ต่อ **F6's `spawn_icon`/`IconRegistry` ยังไม่ถูกเรียกใช้ที่นี่** (ตามที่ F6 ตั้งใจเว้นไว้) — ยังไม่มี icon หน้าชื่อ tab
- **`*Slot` marker entities (`ScenePanelSlot` ฯลฯ) ไม่เปลี่ยนเลย** — `bv_editor_scene_panel`/`bv_editor_inspector_panel` ไม่ต้องแก้โค้ดสักบรรทัด เพราะ slot ยังเป็น entity เดิมที่ถูก `commands.entity(slot).with_children(...)` ได้เหมือนเดิมทุกประการ สิ่งที่เปลี่ยนคือ **`shell.rs` ย้าย (`ChildOf`) slot entity เดิมไปมาระหว่าง leaf ต่างๆ แทนการ despawn/respawn มันทุกครั้งที่ layout เปลี่ยน** — จุดที่ต้องระวังที่สุดตอน implement: ต้อง detach slot ทั้ง 4 ตัว (`remove::<ChildOf>`) ก่อนเสมอ ก่อนจะ despawn chrome เก่า (marked ด้วย component `DockChrome` ที่แยกจาก slot โดยเจตนา) เพราะ `despawn()` เดินตาม hierarchy แบบ recursive — ถ้า detach ช้าไปแม้เฟรมเดียว เนื้อหาจริงของแต่ละ panel (ทั้ง subtree) จะถูกลบไปพร้อม chrome ที่ไม่เกี่ยวกันเลย
- **บั๊กที่เจอระหว่าง implement (ไม่เกี่ยวกับ docking โดยตรง แต่ docking เป็นตัวเปิดโปง):** `splitter_cursor_system`/`scrollbar_cursor_system` (F4/F3) ต่างก็ `remove::<CursorIcon>()` แบบไม่มีเงื่อนไขเมื่อตัวเองไม่มีอะไรจะโชว์ — สอง system นี้รันใน `Update` schedule เดียวกันโดยไม่มีลำดับผูกกันเลย พอเพิ่ม system ของ dock เข้าไปอีกกลุ่ม ลำดับการรันของสอง system เดิมก็เปลี่ยน ทำให้ตัวที่รันทีหลังไปลบ cursor icon ที่อีกตัวเพิ่งตั้งไว้ถูกต้องแล้วทับ (สุ่มพังตามลำดับ scheduler ไม่ใช่บั๊ก logic ตรงๆ) เทส `hovering_a_splitter_sets_the_resize_cursor_and_clears_it_after` จับได้ทันทีที่ลำดับเปลี่ยน — แก้โดยให้ทั้งสอง system เช็คก่อนว่า `CursorIcon` ปัจจุบันเป็นค่าที่ **ตัวเองเคยตั้ง** หรือเปล่า ก่อนจะลบ (ไม่ลบของ system อื่นเด็ดขาด) แทนที่จะพึ่ง registration order ที่ไม่รับประกันเป็นทางการ
- เทสใหม่ 17 เคสใน `dock.rs` (12 pure data-model + 5 ECS-level ของ `dock_drag_drop_system`/`dock_tab_click_system`) ครอบคลุม merge/split/activate/collapse-on-empty/refuse-when-would-empty-region/cross-region ทั้งหมด บวกอัปเดตเทสเดิมใน `bv_editor_ui/src/lib.rs`/`bv_editor/src/lib.rs` ที่เคยอ้างอิง `entities.assets_panel`/`console_panel` หรือ `With<ScenePanelSlot>` แทนความหมายเดิม (ที่ตอนนี้ผูกกับ box ที่ต่างจากตัว content slot แล้ว)
- **แก้บั๊กจริงที่ผู้ใช้เจอหลังลองใช้งาน (2026-09-20): ลาก tab แล้วไม่รวมกันเลย** — สาเหตุคือ `dock_tab_click_system` เดิมทำงานตอน `just_pressed` (เฟรมเดียวกับที่การลากเริ่ม) แล้ว set `DockLayoutDirty` ทันที ทำให้ `rebuild_dock_ui_system` รื้อ chrome ทั้งหมดใหม่ **ระหว่างที่ยังลากอยู่** รวมถึง entity ของปุ่ม tab ที่กำลังถูกกดค้าง ทำให้ drag gesture ขาดตอนตั้งแต่เฟรมแรกที่กดลง — แก้โดยย้ายให้ทำงานตอน `just_released` แทน (rebuild จะไม่เกิดจนกว่า gesture จะจบจริง) เทสใหม่ `dragging_a_dock_tab_onto_another_panel_merges_them_through_the_real_editor_ui_plugin` ใน `bv_editor_ui/src/lib.rs` จำลอง press-hold-release เต็มรูปแบบผ่านระบบจริงทั้ง chain (ไม่ใช่ยิง message เดี่ยวๆ แบบเทสอื่นก่อนหน้า) — ยืนยันแล้วว่าเทสนี้จับบั๊กตัวนี้ได้จริงโดย revert โค้ดกลับไปเป็นแบบเดิมชั่วคราวแล้วดูว่าเทส fail ตรงจุดที่คาด
- **เพิ่ม cursor feedback ตอนลาก tab** (`dock_drag_cursor_system` ใน `shell.rs`) — hover tab strip button → `Grab`, กำลังลากอยู่ → `Grabbing` เหมือน splitter/scrollbar ใช้ `CursorIcon` ค่าเดียวกับ scrollbar โดยเจตนา (ยอมรับ edge case ที่อาจ flicker ข้ามเฟรมได้เมื่อ scrollbar hover พอดีเฟรมเดียวกับที่ dock ไม่มีอะไรจะโชว์ — ดู doc comment ของระบบนี้)

**บั๊กที่ยังไม่แก้ (ค้างไว้ข้ามเซสชัน, พบโดยผู้ใช้ 2026-09-20):**
- **ลากไปวางที่ "ขอบ" (edge-split, ไม่ใช่การรวม tab) ทำให้ panel เป้าหมายเหมือนหายไป** — repro ที่ยืนยันแล้ว: ลาก "Project Files" ไปรวมกับ "Scene Tree" (กลาง tab strip) ได้ผลถูกต้อง จากนั้นลาก "Project Files" กลับไปที่ **ขอบ** ของ "Console" (ไม่ใช่กลาง) เพื่อแยกออกมาเป็นแผงข้างๆ กัน — ผลคือ Console หายไป ผู้ใช้ยืนยันว่าเจาะจงเฉพาะเคส split (ซ้อน panel) เท่านั้น การรวม tab (merge) ทำถูกต้องแล้ว
  - trace `DockLayout::split_onto` ด้วยมือตาม repro นี้แล้วดู logic ถูกต้อง (ควรได้ `Split{axis, first, second}` ที่มีทั้งสอง panel อยู่) — บั๊กน่าจะอยู่ฝั่ง **rendering** ใน `shell.rs` (`render_dock_node`'s Split branch หรือ `render_dock_leaf`'s edge-zone/content-mount) ไม่ใช่ `dock.rs`
  - จุดที่น่าสงสัย (ยังไม่ยืนยัน): `render_dock_leaf` spawn edge-zone entities เป็นลูกของ `mount` **ก่อน** ที่จะ reparent slot entity (เนื้อหาจริง) เข้าไป — sibling หลังมักวาดทับ sibling ก่อนใน `bevy_ui` อาจกระทบทั้ง z-order/hit-test และการคำนวณ layout ของ nested Split ที่อยู่ใต้ region root ที่เป็น `flex_direction: Row` อยู่แล้ว (bottom region)
  - ยังไม่มีเทสอัตโนมัติคลุม edge-split แบบ end-to-end เลย (เทสใน `dock.rs` คลุมแค่ tree mutation ไม่ได้เช็คผล render จริง) — แนะนำเขียนเทสแบบเดียวกับ `dragging_a_dock_tab_onto_another_panel_merges_them_...` ด้านบนแต่จบด้วย `split_onto`/ลากไปขอบจริง แล้วเช็คว่าทั้งสอง `*Slot` มี `Node` ขนาดไม่เป็นศูนย์และอยู่คนละ mount กัน — ถ้าเทส headless ไม่จับ ต้องรัน `ui_gallery` จริงแล้วอ่าน `ComputedNode` สด (แบบเดียวกับที่ F2's scrollbar deadlock bug เคยต้องทำ)
- **ลาก panel ตัวเดียวที่เหลือในโซนไปโซนอื่นถูกปฏิเสธเงียบๆ ไม่มี feedback เลย** — เป็นพฤติกรรมตั้งใจ (guard "โซนห้ามว่างเปล่า" ด้านบน) ไม่ใช่บั๊ก แต่ความเงียบตอนถูกปฏิเสธคือ UX gap จริง ควรมี cue บางอย่าง (เช่น สีแดงตอน hover แทนสีฟ้า) แทนการไม่มีอะไรเกิดขึ้นเลย — ยังไม่ได้ทำ

**ยังไม่ทำ:**
- **persist ลง `editor_layout.ron`** ทั้งของ F5 เองและของ F4 ("จำขนาด panel ข้ามเซสชัน") — ยังผูก scope เดียวกันตามที่ร่างไว้เดิม ยังไม่เริ่ม ตรงกับ Phase 10 เดิมของ DESIGN.md
- **ไม่มีการ reorder tab ภายใน leaf เดียวกันแบบตั้งใจ** — ลาก tab ไปวางที่ leaf ของตัวเองได้ผลเป็นการย้ายไปท้ายแถว (ผลข้างเคียงของ `merge_onto`'s guard `dragged == target_anchor` no-op เฉพาะกรณีลากลงบนตัวเอง ไม่ใช่ฟีเจอร์ที่ตั้งใจออกแบบ)
- **floating window นอกหน้าต่างหลัก** — นอกสโคป v1 เดิม ไม่เปลี่ยน
- **F6's icon ในหน้า tab** — โครงสร้าง `IconRegistry` พร้อมใช้แล้ว แค่ยังไม่ถูกเรียกจาก tab strip button
- **feedback ตอนถูกปฏิเสธเพราะจะทำให้โซนว่าง** — ดูหัวข้อ "บั๊กที่ยังไม่แก้" ด้านบน

**สเปกเดิม (สำหรับอ้างอิง):**

**สถานะตอนร่างสเปก (ก่อน implement):** ยังไม่มี — คอมเมนต์ใน `splitter.rs` ระบุไว้ตรงๆ ว่า "moving/undocking panels ('docking' proper) is out of scope until Phase 10" ตรงกับที่ DESIGN.md section 10 (ความเสี่ยง) พูดถึงไว้ว่า docking framework เป็นความเสี่ยงหลักของโปรเจกต์เพราะ `bevy_ui` ไม่มีสำเร็จรูป นี่คือฟีเจอร์ก้อนใหญ่ที่สุดในคำขอทั้ง 6 ข้อ
...
7. ~~**F5** (docking)~~ ✅ เสร็จแล้วเฉพาะแกนหลัก (merge/split/resize ข้ามโซน) — **ยังไม่ persist ลง `editor_layout.ron`** ยังผูก scope เดียวกับ F4's "จำขนาดข้ามเซสชัน" ที่ยังไม่เริ่ม ทั้งคู่เหลือเป็นงานที่แท้จริงของ Phase 10
