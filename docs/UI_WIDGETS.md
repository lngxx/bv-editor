...
| **ScrollArea** | [`bv_editor_ui::scroll_area::spawn_scroll_area`](../crates/bv_editor_ui/src/scroll_area.rs) | Widget ประกอบสำเร็จรูป — เรียกครั้งเดียวได้ container ที่ scroll ได้ (`Overflow`) + track/thumb ตาม `ScrollAxes` (`Vertical`/`Horizontal`/`Both`) พร้อมกัน คืน entity ของ "body" ให้ caller ใส่ marker component ของตัวเองต่อ (`commands.entity(body).insert(YourMarker)`) — สกัดออกมาจาก `bv_editor_scene_panel`/`bv_editor_inspector_panel` ที่เคยเขียนโครงสร้างนี้แยกกันเอง 2 ที่ (แก้บั๊กซ้ำสองรอบเพราะงั้น) | ใช้งานได้ (F2/F3 ✅) — Scene Tree ใช้ `Vertical`, Components panel ใช้ `Both` |
| **Scrollbar** | [`bv_editor_ui::scrollbar`](../crates/bv_editor_ui/src/scrollbar.rs) | `ScrollbarThumb { target, axis }` ลากด้วยเมาส์ (แนวตั้งหรือแนวนอนตาม `axis`) + wheel scroll (แนวตั้งอย่างเดียว) เมื่อ hover container ที่ตั้ง `Overflow` แกนนั้นเป็น scroll ไว้ — ซ่อนทั้ง track และ thumb อัตโนมัติเมื่อเนื้อหาพอดีพื้นที่, hover/ลากเปลี่ยน mouse cursor เป็น grab/grabbing ผ่าน `bevy_window::CursorIcon`, track หนา 12px (`flex_shrink: 0.0` กันโดน flexbox บีบ) ไม่ใช้ `bevy_ui_widgets::scrollbar` (ของจริงมีในเวอร์ชันนี้ แต่สร้างบน `bevy_picking`, คนละ paradigm กับ widget อื่นในนี้) — เป็น engine เบื้องหลังของ **ScrollArea** ด้านบน ไม่ได้ใช้ตรงๆ แยกเดี่ยวๆ อีกแล้ว | ใช้งานได้ (F2/F3 ✅) |
